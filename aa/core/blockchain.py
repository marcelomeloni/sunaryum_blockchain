...
class Blockchain:
    ...
    def create_block(self, transactions, proposer):
        ...
        # Criar cópia simplificada das transações (remover campos específicos)
        simplified_txs = []
        for tx in transactions:
            ...
            # Mantém o campo proposer se existir
            if "proposer" in tx:
                simplified_tx["proposer"] = tx["proposer"]
            simplified_txs.append(simplified_tx)
        ...
        block = {
            ...
            "proposer": proposer,
            "transactions": simplified_txs  # Usar transações simplificadas
        }

        block["hash"] = self.calculate_block_hash(block)
        return block
    def validate_transaction(self, tx, utxo_set):
        # 1. Verificar se é uma transação coinbase (recompensa)
        if not tx.get('inputs') and tx.get('outputs'):
            logger.debug("Transação coinbase validada")
            return True
    
        # 2. Verificar assinatura para transações regulares
        if not self.wallet.verify_transaction(tx):
            logger.error(f"Assinatura inválida na transação: {tx.get('id')}")
            return False
    
        # 3. Verificar inputs apenas para transações não-coinbase
        input_sum = 0
        ...
        # 5. Verificar se inputs cobrem outputs
