...
def elect_proposer(nodes, seed, threshold, day_index):
    ...
    for i, (public_key, output, proof) in enumerate(nodes):
        ...
        try:
            # 2. Converter output para valor numérico
            output_bytes = bytes.fromhex(output[:64])  # Usar apenas 64 caracteres hex (256 bits)
            output_int = int.from_bytes(output_bytes, 'big')
            max_value = (1 << 256) - 1
            normalized = output_int / max_value
            
            logger.debug(f"  VRF normalizado: {normalized:.6f} (threshold: {threshold})")
            
            # 3. Verificar se está abaixo do threshold
